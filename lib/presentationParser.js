...
function presentationParser(entries, pptx, relations) {
	...
	for(let i=1; i<10; i++) {
		...
		presentation['defaultTextStyle']['level' + i + 'ParagraphProperty']['size'] = drp.$.sz;
		presentation['defaultTextStyle']['level' + i + 'ParagraphProperty']['kern'] = drp.$.kern;
		presentation['defaultTextStyle']['level' + i + 'ParagraphProperty']['color'] = drp['a:solidFill'][0]['a:schemeClr'][0].$.val;
	}
