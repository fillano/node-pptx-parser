...
function summarize(relations, presentation, themes, slideMasters, slideLayouts, slides) {
	...
	ret.slides = presentation.slideIdList.reduce((pre, cur) => {
		...
		.filter(z => (z.spType === null && z.spIdx === null) || z.type === 'pic');
		...
		.reduce((pre, cur) => {
			if(!!cur.spType) {
				if(cur.spIdx === null) {
					...
					let masterShape = master.commonSlideData.shapes.filter(z => z.spType === 'body')[0];
					pre.push(processShape(cur, layoutShape, masterShape, style));
				}
			} else {
				if(!!cur.spIdx) {
		...
		ret.commonSlideData.shapes = backgroundShapes.concat(placeholderShapes).concat(customShapes);
...
function mergeShape(base, target) {
	let ret = {};
	...
	if(!!ret.textBody && !!ret.textBody.paragraphs && ret.textBody.paragraphs.length > 0) {
		...
		ret.textBody.paragraphs = ret.textBody.paragraphs.reduce((pre, cur) => {
			...
			if(!!tmp.runs) {
				tmp.runs = tmp.runs.reduce((pre1, cur1) => {
					if(cur1.type === 'r') {
						pre1.push(deepAssign(ret.textBody.defaultListStyle['level'+level+'ParagraphProperty'].defRPr, cur1));
					} else {
						pre1.push(cur);
					}
					return pre1;
